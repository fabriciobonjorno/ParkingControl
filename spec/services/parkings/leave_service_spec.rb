...
RSpec.describe Parkings::LeaveService do
  describe '.call' do
    context 'with paid parking' do
      let!(:parking) { create(:parking, :paid, plate: 'ABC-1234') }

      it 'marks parking as left' do
        freeze_time do
          described_class.call(plate: 'ABC-1234')
          expect(parking.reload.left_at).to be_within(1.second).of(Time.current)
        end
      end
    ...
    context 'when parking is not paid' do
      ...
      it 'raises error' do
        expect {
          described_class.call(plate: 'ABC-1234')
        }.to raise_error(StandardError, /Pagamento não realizado/)
      end
    end
